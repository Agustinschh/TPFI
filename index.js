    ...
    scanDb(contacto)
        .then(resultDb => {
            if (Object.keys(resultDb).length != 0) {
                ...
                const newCliente = {
                    id: uuidv4(),  // Generar UUID en lugar de usar el contacto como ID
                    nombre: nombre,
                    contacto: contacto,  // Mantener el contacto como un campo separado
                    password: password,
                    activo: true,
                    registrado: true,
...
app.get('/api/listarTickets', (req, res) => {
    scanDBTickets()
        .then(resultDb => {
            console.log(resultDb);
            if (Object.keys(resultDb).length === 0) {
        ...
        .catch(error => {
            ...
            res.status(500).send({ response: "ERROR", message: "Error en el servidor" });
        });
});
...
async function scanDBTickets() {
    try {
        const input = {
            TableName: "ticket", // Cambia esto por el nombre de tu tabla de tickets
            Select: "SPECIFIC_ATTRIBUTES",
            AttributesToGet: [
                'id', 'clienteID', 'estado_solucion', 'solucion', 'descripcion', 'fecha_apertura', 'ultimo_contacto' // Campos que quieres obtener
            ],
        };
...
app.get('/api/loginClienteEmail', (req, res) => {
    const { contacto, password } = req.query;

    console.log("Valores recibidos:", contacto, password);  // <-- Agrega esto para validar los valores

    if (!password || !contacto) {
        res.status(400).send({ response: "ERROR", message: "Debe informar contacto y password" });
        return;
    }
    ...
    docClient.scan(paramsScan, (err, data) => {
        if (err) {
            res.status(400).send({ response: "ERROR", message: "DB access error " + err });
        } else if (data.Items.length == 0) {
            res.status(400).send({ response: "invalid" });
        } else {
            const cliente = data.Items[0];
            if (cliente.password === password && cliente.activo && cliente.registrado) {
                res.status(200).send({
                    response: "OK",
                    id: cliente.id,
